...
template <int Dim>
planning_ros_msgs::PathArray path_array_to_ros( const vec_E<vec_Vecf<Dim>> &paths, double h = 0)
{
...
template <int Dim>
planning_ros_msgs::PathArray path_array_to_ros( const std::vector<std::pair<std::string, vec_Vecf<Dim>>> &paths, double h = 0)
{
    planning_ros_msgs::PathArray msg;
    for (const auto &it : paths)
    {
