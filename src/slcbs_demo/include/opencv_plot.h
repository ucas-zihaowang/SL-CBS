...
public:
    ...
    void drawSolidRect( const Vec2f &pt1, const Vec2f &pt2, cv::Scalar color, int line_width = 1 )
    {
        const auto pn1 = gmap.toGrid(pt1);
        cv::Point ptn1(pn1(0), pn1(1));
        const auto pn2 = gmap.toGrid(pt2);
        cv::Point ptn2(pn2(0), pn2(1));
        cv::rectangle( img, ptn1, ptn2, color, cv::FILLED );
    }
...
//            drawCircle( curr_expand_state.pos, red, 2, 1);
            ...
            for( int j = 0; j < wss_size; j++ )
            {
                ...
                int ws_size = ws.size();
                cv::Scalar color_select = magenta;
