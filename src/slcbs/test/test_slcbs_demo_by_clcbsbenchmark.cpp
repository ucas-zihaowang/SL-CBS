...
static double ObsRadius = 0.5;
static double UAVRadius = 1.0;
...
{
    std::string name = "test_slcbs_demo_by_clcbsbenchmark_node";
    ...
    if ( res )
    {
        ...
        {
            totalcost += sols[i].totalcost;
            double tmpcost = sols[i].actions.size() * cp.dt;
            tcost += tmpcost;
            if ( makespan < tmpcost)
                makespan = tmpcost;
        }
        resultByCount << "  - runtime: " << spend_time / 1000.0f << std::endl;
        resultByCount << "    timecost: " <<  tcost << std::endl;
        resultByCount << "    makespan: " <<  makespan << std::endl;
        resultByCount << "    totalcost: " <<  totalcost << std::endl;
    }
    else
    {
